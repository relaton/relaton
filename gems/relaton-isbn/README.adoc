...
[NOTE]
====
`relaton-isbn` is distributed as part of the combined `relaton` gem and is no longer published as a standalone gem.
Install `relaton` (it bundles every flavor, loaded lazily) instead of
`relaton-isbn`. This directory is the gem's development home in the
