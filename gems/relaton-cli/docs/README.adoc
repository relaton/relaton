...
The generated `index.html` is self-contained: it inlines a compiled
Vue + Tailwind frontend (search, doctype/stage filters, sort, list/grid,
dark mode, copy-DocID, responsive pagination) and carries the document data so no server is
required. Output is written to `_site/` (`index.html` + `search.json`).
...
edition, languages, keywords, publisher, and the landing/raw-YAML links. These
richer fields are embedded in the page (in `embedded` mode) so the detail page
works offline with no extra requests; a `dom`/`static-json` build shows the
summary fields and links.
...
| `--favicon` | Favicon href for `<link rel="icon">`, emitted **verbatim**. Normally an absolute URL (e.g. the SDO's own logo). A site-relative path also works, but `relaton index` copies no assets — only `index.html` and `search.json` are written — so the caller must put that file into the deployed site itself. A known image extension (`.svg`, `.png`, `.ico`, `.gif`, `.jpg`) also sets `type`; anything else is left for the browser to sniff.
| `--base-url` | Base URL for the raw-YAML links (defaults to the raw GitHub URL when cloning).
| `-m, --mode` | Data delivery mode (default `embedded`):

  * `embedded` — crawler-indexable DOM **and** an embedded JSON payload (recommended).
  * `dom` — crawler-indexable DOM only.
  * `static-json` — a `search.json` sidecar the page fetches (not crawler-indexable).
...
# Emit a fetched search.json sidecar instead of the embedded payload:
$ relaton index data --mode static-json --output _site
----
