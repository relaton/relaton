...
Entries are retrieved and stored in the https://github.com/riboseinc/relaton-models[Relaton bibliographic model], which is an expression of ISO 690. The subset of the model used and serialised for Relaton is defined in the https://github.com/metanorma/iso-bib-item[iso-bib-item] gem.

Entries are serialised to and from an internal data model, and multiple formats are intended to be supported. Currently only https://github.com/riboseinc/relaton-models/blob/master/grammars/biblio.rnc[Relaton XML] is supported.
