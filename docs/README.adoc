...
* `globalcache` - path to globalcache directory
* `localcache` - path to localcache directory
...
There are 2 fetching methods:

* `Relaton::Db#fetch(reference, year, options)` - fetches document immediately
* `Relaton::Db#fetch_async(reference, year, options, &block)` - fetches document asynchronously

Arguments:

* `reference` - reference to fethc document
* `year` - (String) year to filter relult (optional)
* `options` - hash of options. Alloved options:
