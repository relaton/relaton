  ...
  context "log" do
    ...
    context "log to file" do
      it "use string formatter" do
        Relaton::Logger.configure do |config|
          config.logger_pool[:default] = Relaton::Logger::Log.new("fixtures/log.log")
        end
        Relaton.logger_pool.info "Test log", "Prog Name", key: "Key"
        expect(File.read("fixtures/log.log")).to match(/\[Prog Name\] INFO: \(Key\) Test log/)
        Relaton.logger_pool.truncate
        expect(File.read("fixtures/log.log")).to eq ""
      end

      it "use json formatter" do
        Relaton::Logger.configure do |config|
          config.logger_pool[:default] = Relaton::Logger::Log.new(
            "fixtures/log.json", formatter: Relaton::Logger::FormatterJSON
          )
        end
        Relaton.logger_pool.truncate
        expect(File.read("fixtures/log.json")).to eq ""
        Relaton.logger_pool.info "Test log", "Prog Name", key: "Key1"
        Relaton.logger_pool.info "Test log", "Prog Name", key: "Key2"
        log = File.readlines("fixtures/log.json").map { |l| JSON.parse l }
        expect(log).to eq [
