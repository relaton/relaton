RSpec.describe Relaton::Db do
  ...
  context "gets an ISO reference" do
    it "and caches it" do
      ...
      item = Relaton::Iso::ItemData.new docid: [docid], fetched: Date.today.to_s
      expect(Relaton::Iso::Bibliography).to receive(:get).with("ISO 19115-1", nil, {}).and_return item
  ...
  context "IEC" do
    before do
      docid = Relaton::Bib::Docidentifier.new(content: "IEC 60050-102:2007", type: "IEC")
      item = Relaton::Iec::ItemData.new docidentifier: [docid]
      expect(Relaton::Iec::Bibliography).to receive(:get).with("IEC 60050-102:2007", nil, {}).and_return item
    end
  ...
  context "NIST references" do
    it "gets FISP" do
      ...
      docid = Relaton::Bib::Docidentifier.new(content: "NIST FIPS 140", type: "NIST")
      item = Relaton::Nist::ItemData.new docidentifier: [docid]
      expect(Relaton::Nist::Bibliography).to receive(:get).with("NIST FIPS 140", nil, {}).and_return item
    ...
    it "gets SP" do
      require "relaton/nist"
      docid = Relaton::Bib::Docidentifier.new(content: "NIST SP 800-38B", type: "NIST")
      item = Relaton::Nist::ItemData.new docidentifier: [docid]
      expect(Relaton::Nist::Bibliography).to receive(:get).with("NIST SP 800-38B", nil, {}).and_return item
  ...
  it "get CCSDS reference" do
    ...
    docid = Relaton::Bib::Docidentifier.new content: "CCSDS 230.2-G-1", type: "CCSDS"
    item = Relaton::Ccsds::ItemData.new docidentifier: [docid]
    expect(Relaton::Ccsds::Bibliography).to receive(:get).with("CCSDS 230.2-G-1", nil, {}).and_return item
  ...
  it "get IHO reference" do
    ...
    expect(Relaton::Iho::Bibliography).to receive(:get).with("IHO B-11", nil, {}).and_return item
  ...
  it "get BSI reference" do
    require "relaton/bsi"
    docid = Relaton::Bib::Docidentifier.new(content: "BSI BS EN ISO 8848", type: "BSI")
    item = Relaton::Bsi::ItemData.new docidentifier: [docid]
    expect(Relaton::Bsi::Bibliography).to receive(:get).with("BSI BS EN ISO 8848", nil, {}).and_return item
  ...
  it "get IANA reference" do
    ...
    docid = Relaton::Bib::Docidentifier.new(content: "IANA service-names-port-numbers", type: "IANA")
    item = Relaton::Iana::ItemData.new docidentifier: [docid]
    expect(Relaton::Iana::Bibliography).to receive(:get).with("IANA service-names-port-numbers", nil, {}).and_return item
  ...
  it "get OASIS reference" do
    ...
    docid = Relaton::Bib::Docidentifier.new(content: "OASIS amqp-core-types-v1.0-Pt1", type: "OASIS")
    item = Relaton::Oasis::ItemData.new docidentifier: [docid]
    expect(Relaton::Oasis::Bibliography).to receive(:get).with("OASIS amqp-core-types-v1.0-Pt1", nil, {}).and_return item
  ...
  it "get BIPM reference" do
    ...
    docid = Relaton::Bib::Docidentifier.new(content: "BIPM Metrologia 29 6 373", type: "BIPM")
    item = Relaton::Bipm::ItemData.new docidentifier: [docid]
    expect(Relaton::Bipm::Bibliography).to receive(:get).with("BIPM Metrologia 29 6 373", nil, {}).and_return item
  ...
  it "get JIS reference" do
    ...
    expect(Relaton::Jis::Bibliography).to receive(:get).with("JIS X 0001", nil, {}).and_return item
  ...
  it "get ETSI reference" do
    ...
    docid = Relaton::Bib::Docidentifier.new(content: "ETSI EN 300 175-8", type: "ETSI")
    item = Relaton::Etsi::ItemData.new docidentifier: [docid]
    expect(Relaton::Etsi::Bibliography).to receive(:get).with("ETSI EN 300 175-8", nil, {}).and_return item
  ...
  it "get PLATEAU reference" do
    ...
    docid = Relaton::Bib::Docidentifier.new(content: "PLATEAU Hanbook #01", type: "PLATEAU")
  ...
  context "get combined documents" do
    context "ISO" do
      it "included" do
        VCR.use_cassette "iso_combined_included" do
          ...
          expect(bib.docidentifier[0].content).to eq "ISO 19115-1:2014 + Amd 1"
          expect(bib.relation[0].type).to eq "updates"
          expect(bib.relation[0].bibitem.docidentifier[0].content).to eq "ISO 19115-1:2014"
          expect(bib.relation[1].type).to eq "derivedFrom"
          expect(bib.relation[1].description).to be_nil
          expect(bib.relation[1].bibitem.docidentifier[0].content).to eq "ISO 19115-1:2014/Amd 1:2018"
        end
      end
    ...
    context "IEC" do
      it "included" do
        item = Relaton::Iec::ItemData.new docidentifier: [Relaton::Iec::Docidentifier.new(content: "IEC 60027-1", type: "IEC")]
        expect(Relaton::Iec::Bibliography).to receive(:get).with("IEC 60027-1", nil, {}).and_return item
        item1 = Relaton::Iec::ItemData.new docidentifier: [Relaton::Iec::Docidentifier.new(content: "IEC 60027-1/AMD1:1997", type: "IEC")]
        expect(Relaton::Iec::Bibliography).to receive(:get).with("IEC 60027-1/Amd 1", nil, {}).and_return item1
        item2 = Relaton::Iec::ItemData.new docidentifier: [Relaton::Iec::Docidentifier.new(content: "IEC 60027-1/AMD2:2005", type: "IEC")]
        expect(Relaton::Iec::Bibliography).to receive(:get).with("IEC 60027-1/Amd 2", nil, {}).and_return item2
        bib = @db.fetch "IEC 60027-1, Amd 1, Amd 2"
        expect(bib.docidentifier[0].content).to eq "IEC 60027-1, Amd 1, Amd 2"
        expect(bib.relation[0].type).to eq "updates"
        expect(bib.relation[0].bibitem.docidentifier[0].content).to eq "IEC 60027-1"
        expect(bib.relation[1].type).to eq "complements"
        expect(bib.relation[1].description.content).to eq "amendment"
        expect(bib.relation[1].bibitem.docidentifier[0].content).to eq "IEC 60027-1/AMD1:1997"
        expect(bib.relation[2].type).to eq "complements"
        expect(bib.relation[2].description.content).to eq "amendment"
        expect(bib.relation[2].bibitem.docidentifier[0].content).to eq "IEC 60027-1/AMD2:2005"
      end
    end

    context "ITU" do
      it "included" do
        docid = Relaton::Bib::Docidentifier.new(content: "ITU-T G.989.2", type: "ITU")
        ...
        expect(Relaton::Itu::Bibliography).to receive(:get).with("ITU-T G.989.2", nil, {}).and_return item
        docid1 = Relaton::Bib::Docidentifier.new(content: "ITU-T G.989.2 Amd 1", type: "ITU")
        item1 = Relaton::Itu::ItemData.new(
          docidentifier: [docid1], contributor: [contrib],
        )
        expect(Relaton::Itu::Bibliography).to receive(:get).with("ITU-T G.989.2 Amd 1", nil, {}).and_return item1
        docid2 = Relaton::Bib::Docidentifier.new(content: "ITU-T G.989.2 Amd 2", type: "ITU")
        item2 = Relaton::Itu::ItemData.new(
          docidentifier: [docid2], contributor: [contrib],
        )
        expect(Relaton::Itu::Bibliography).to receive(:get).with("ITU-T G.989.2 Amd 2", nil, {}).and_return item2
        bib = @db.fetch "ITU-T G.989.2, Amd 1, Amd 2"
        expect(bib.docidentifier[0].content).to eq "ITU-T G.989.2, Amd 1, Amd 2"
        expect(bib.relation[0].type).to eq "updates"
        expect(bib.relation[0].bibitem.docidentifier[0].content).to eq "ITU-T G.989.2"
        expect(bib.relation[1].type).to eq "complements"
        expect(bib.relation[1].description.content).to eq "amendment"
        expect(bib.relation[1].bibitem.docidentifier[0].content).to eq "ITU-T G.989.2 Amd 1"
        expect(bib.relation[2].type).to eq "complements"
        expect(bib.relation[2].description.content).to eq "amendment"
        expect(bib.relation[2].bibitem.docidentifier[0].content).to eq "ITU-T G.989.2 Amd 2"
      end
    end
    ...
    context "NIST" do
      it "included" do
        ...
        require "relaton/nist"
        doci = Relaton::Bib::Docidentifier.new(content: "SP 800-38A", type: "NIST")
        item = Relaton::Nist::ItemData.new docidentifier: [doci]
        expect(Relaton::Nist::Bibliography).to receive(:get).with("NIST SP 800-38A", nil, {}).and_return item
        docid1 = Relaton::Bib::Docidentifier.new(content: "SP 800-38A-Add", type: "NIST")
        item1 = Relaton::Nist::ItemData.new docidentifier: [docid1]
        expect(Relaton::Nist::Bibliography).to receive(:get).with("NIST SP 800-38A/Add", nil, {}).and_return item1
        bib = @db.fetch "NIST SP 800-38A, Add"
        expect(bib.docidentifier[0].content).to eq "NIST SP 800-38A, Add"
        expect(bib.relation[0].type).to eq "updates"
        expect(bib.relation[0].bibitem.docidentifier[0].content).to eq "SP 800-38A"
        expect(bib.relation[1].type).to eq "complements"
        expect(bib.relation[1].description.content).to eq "amendment"
        expect(bib.relation[1].bibitem.docidentifier[0].content).to eq "SP 800-38A-Add"
        # end
      end
    end
  end
