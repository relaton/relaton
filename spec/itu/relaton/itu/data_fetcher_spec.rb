    ...
    context "#fetch itu-r routing" do
      ...
      [nil, "itu-r"].each do |source|
        it "harvests every implemented family for source #{source.inspect}" do
          ...
          families.each do |f|
            expect(crawler).to receive(:harvest).with("X", hash_including(family: f)).and_return [:item]
          end
          expect(Relaton::Itu::DataMergeR).to receive(:write_all).exactly(families.size).times
      ...
      it "tallies the merge across families" do
        ...
        # one series per family, so every count scales with the family count
        expect(subject.fetch_publications)
