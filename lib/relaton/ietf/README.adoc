= Relaton::Ietf

_Formerly known as_ `rfcbib`.

Relaton::Ietf is a Ruby gem that searches and fetches standards from the https://www.ietf.org[Internet Engineering Task Force (IETF)], including RFCs.

The standards scrape form https://www.rfc-editor.org/search/rfc_search.php

== Usage

=== Fetching documents

[source,ruby]
----
require 'relaton/ietf'
=> true

# Fetch RFC document
item = Relaton::Ietf::Bibliography.get 'IETF RFC 8341'
relaton-ietf] INFO: (IETF RFC 8341) Fetching from Relaton repository ...
[relaton-ietf] INFO: (IETF RFC 8341) Found: `RFC 8341`
=> #<Relaton::Ietf::ItemData:0x0000000125608cb0
...

# Fetch Internet-Draft document
Relaton::Ietf::Bibliography.get 'IETF I-D.draft-abarth-cake-01'
[relaton-ietf] INFO: (IETF I-D.draft-abarth-cake-01) Fetching from Relaton repository ...
[relaton-ietf] INFO: (IETF I-D.draft-abarth-cake-01) Found: `draft-abarth-cake-01`
=> #<Relaton::Ietf::ItemData:0x000000012141fdd0
...

# Return nil if a document doesn't exist.
Relaton::Ietf::Bibliography.get 'IETF 1111'
[relaton-ietf] INFO: (IETF 1111) Fetching from Relaton repository ...
[relaton-ietf] INFO: (IETF 1111) Not found.
=> nil
----

=== Serialization

[source,ruby]
----
item.to_xml
=> "<bibitem id="RFC8341" type="standard" schema-version="v1.4.1">
      <fetched>2026-02-18</fetched>
      <title type="main">Network Configuration Access Control Model</title>
      <uri type="src">https://www.rfc-editor.org/info/rfc8341</uri>
      <docidentifier type="IETF" primary="true">RFC 8341</docidentifier>
      ....
    </bibitem>"
----
With `bibdata: true` option XML output is wrapped with `bibdata` element and `ext` element added.
[source,ruby]
----
item.to_xml bibdata: true
=> "<bibdata type="standard" schema-version="v1.4.1">
      <fetched>2026-02-18</fetched>
      <title type="main">Network Configuration Access Control Model</title>
      <uri type="src">https://www.rfc-editor.org/info/rfc8341</uri>
      <docidentifier type="IETF" primary="true">RFC 8341</docidentifier>
      ...
      <ext schema-version="v1.0.1">
        <doctype>rfc</doctype>
        <flavor>ietf</flavor>
        <stream>IETF</stream>
      </ext>
    </bibdata>"
----

=== Typed source links

IETF documents may have `src`, `xml`, and `doi` source link types.

* `src` - web publication
* `xml` - BibXML publication
* `doi` - DOI reference

[source,ruby]
----
item.source.first.type
=> "src"
item.source.first.content
=> "https://www.rfc-editor.org/info/rfc8341"
----

=== Parse a file locally

[source,ruby]
----
item = Relaton::Ietf::BibXMLParser.parse_rfc File.read("spec/ietf/fixtures/rfc.xml")
=> #<Relaton::Ietf::ItemData:0x000000012913e4d8
...
----

=== Create bibliographic item from YAML
[source,ruby]
----
Relaton::Ietf::Item.from_yaml File.read("spec/ietf/fixtures/item.yaml")
=> #<Relaton::Ietf::ItemData:0x0000000127edf508
...
----

=== Fetch data

There are IETF datasets that can be converted into RelatonXML/BibXML/BibYAML formats:

- `ietf-rfcsubseries` - https://www.rfc-editor.org/rfc-index.xml (`<bcp-entry>`, `<fyi-entry>`, `<std-entry>`)
- `ietf-internet-drafts` - convert files from local directory `./bibxml-ids`. Use `rsync -avcizxL rsync.ietf.org::bibxml-ids ./bibxml-ids` command to fetch the files.
- `ietf-rfc-entries` - https://www.rfc-editor.org/rfc-index.xml (`<rfc-entry>`)

The method `Relaton::Ietf::DataFetcher.fetch(source, output: "data", format: "yaml")` converts all the documents from the dataset and saves them to the `./data` folder in YAML format.

Arguments:

- `source` - dataset name (`ietf-rfcsubseries` or `ietf-internet-drafts`)
- `output` - folder to save documents (default './data').
- `format` - the format in which the documents are saved. Possible formats are: `yaml`, `xml`, `bibxml` (default `yaml`).

[source,ruby]
----
Relaton::Ietf::DataFetcher.fetch "ietf-internet-drafts"
Started at: 2021-12-17 10:23:20 +0100
Stopped at: 2021-12-17 10:29:19 +0100
Done in: 1360 sec.
=> nil
----

=== The index

Lookups read one combined index — `index-v2.zip` from
https://github.com/relaton/relaton-data-ietf[relaton-data-ietf], covering RFCs,
the RFC sub-series and Internet-Drafts in ~177k rows. `DataFetcher` writes the same
shape as it crawls, one row per record keyed by that record's primary
docidentifier, serialised as a `Pubid::Ietf::Identifier`:

[source,yaml]
----
- :id:
    _type: pubid:ietf:rfc
    number: '3986'
  :file: data/rfc3986.yaml
----

A record whose identifier `Pubid::Ietf` cannot parse is still written, but is left
out of the index and logged — a single unparseable row would otherwise make the
whole index unloadable for every consumer. The total is restated when the crawl
finishes.

Note sub-series ids key to their own record: `STD 66` resolves to the STD document,
whose `relation: includes` names the RFC. There is no single RFC to alias to in
general — 42 of the 365 sub-series include more than one.

=== Logging

Relaton::Ietf uses the relaton-logger gem for logging. By default, it logs to STDOUT. To change the log levels and add other loggers, read the https://github.com/relaton/relaton-logger#usage[relaton-logger] documentation.

== Development

The suite uses one cached index fixture, `spec/ietf/fixtures/ietf-index-v2.zip`, copied wholesale from https://github.com/relaton/relaton-data-ietf[relaton-data-ietf]. Refresh it by re-downloading that repo's `index-v2.zip` when a document lookup starts 404ing — a stale index names files that no longer exist, which surfaces as a 404 rather than as an index error.

== Contributing

Bug reports and pull requests are welcome on GitHub at https://github.com/metanorma/relaton-ietf.

== License

The gem is available as open source under the terms of the [MIT License](https://opensource.org/licenses/MIT).
