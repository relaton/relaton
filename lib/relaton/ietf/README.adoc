...
`DataFetcher` writes `index-v2.yaml` alongside the documents as it crawls, one row
per record keyed by that record's primary docidentifier, serialised as a
...
The suite uses three cached index fixtures, `spec/ietf/fixtures/{rfc,rss,ids}-index-v1.zip`, each a near-complete older snapshot of the published index of https://github.com/relaton/relaton-data-rfcs[relaton-data-rfcs], https://github.com/relaton/relaton-data-rfcsubseries[relaton-data-rfcsubseries], and https://github.com/relaton/relaton-data-ids[relaton-data-ids] respectively. Refresh them by hand from those repos' `index-v1.zip` on branch `v2` when a document lookup starts 404ing.
