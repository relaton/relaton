...
For the data-repository layout, the index file specification (schema, `:id` v1/v2 shapes), and the GitHub Pages publishing contract, see link:../../../docs/data-repository-format.adoc[docs/data-repository-format.adoc] at the repo root.

== Usage

=== Creating an index object

The gem provides the `Relaton::Index.find_or_create {type}, url: {url}, file: {filename}, pubid_class: {class}` method to create an index object. The first argument is the type of dataset (ISO, IEC, IHO, etc.). `url` is the URL to the zipped remote index file, `filename` is the local index file, and `pubid_class` is a class that implements `Pubid::Core::Identifier` for deserializing ID hashes into structured identifier objects. All three keywords are optional.
...
Relaton::Index.find_or_create :IHO, url: "https://raw.githubusercontent.com/relaton/relaton-data-iho/master/index.zip", file: "index-iho.yaml"
...
A string reference matches a row as a substring, as above. An identifier
reference takes pubid's asymmetric subset match `reference === row[:id]`: a
component the reference leaves out matches any value, so `OGC 12-128` answers
with the `12-128r19` row. Pass a block for exact equality:

[source,ruby]
----
index.search(pubid) { |row| row[:id] == pubid }
----
...
=== Reading the machine index on a Pages site

A data repository's GitHub Pages site publishes a sharded copy of its index
(`index/manifest.json` and `index/shard-NNNNN.json`, see
`docs/data-repository-format.adoc`). Give its base URL as `pages_url:` in place
of `url:`, with a `pubid_class:`:
...
index.search(Pubid::W3c::Identifier.parse("W3C REC-xml-names"))
----

A parsed query reads the manifest and the one shard
`crc32(id.root.number.to_s) % shards`. A String query and a search with only a
block read the whole index, which the manifest names. Nothing is written to
disk: the manifest, the shards and the whole index stay in memory for 24 hours.

* An absent shard (HTTP 404) is "not found", and the search returns `[]`.
* A network error, or an HTTP status other than 200 and 404, raises
  `Relaton::RequestError`.
* A missing or unsupported manifest, or a row that the pubid class cannot read,
  raises `Relaton::Index::Error`.

There is no fallback to the `url:` zip.
