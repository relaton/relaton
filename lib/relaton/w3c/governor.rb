...
module Relaton
  module W3c
    #
    # Process-wide, thread-safe back-pressure for the W3C crawl.
    #
    # api.w3.org sits behind Cloudflare, whose rate limiting is bimodal: a run
    # either never trips it or trips it and stays banned for hours. Before this
    ...
    class Governor
      ...
      attr_reader :base, :max, :give_up_after

      def initialize(base: nil, max: nil, give_up_after: nil,
                     clock: nil, sleeper: nil, jitter: nil)
        ...
        @jitter = jitter || ->(secs) { Kernel.rand * secs * JITTER_FRACTION }
        @mutex = Mutex.new
        @cooldown_until = nil
        @penalty = nil
        @rounds = 0
      ...
      # environment. Re-read on #reset! so the env vars behave like the sibling
      # RELATON_W3C_* knobs, which are class methods evaluated per crawl rather
      # than frozen when the flavor was autoloaded.
      def load_limits
        @base = @overrides[:base] || self.class.env_int("RELATON_W3C_THROTTLE_BASE", BASE_COOLDOWN)
        @max = @overrides[:max] || self.class.env_int("RELATON_W3C_THROTTLE_MAX", MAX_COOLDOWN)
        @give_up_after = @overrides[:give_up_after] ||
          self.class.env_int("RELATON_W3C_THROTTLE_GIVEUP", GIVE_UP_AFTER)
      end
