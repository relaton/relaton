...
- **The HAL object cache is process-global, and the suite clears it per example.**
  `W3cApi::Hal` is a Singleton holding one `Lutaml::Hal::ModelRegister` with an
  in-memory URL cache, registered in the `Lutaml::Hal::GlobalRegister` Singleton.
  `W3cApi::Client.new` carries no state, so a fresh client per example still
  reads that one cache. Left warm it chains the examples together:
  `DataParser#parse` realizes `@spec.links.specification` for the `editionOf`
  relation, so the `vcr: "webrtc-20241008"` examples fetch
  `https://api.w3.org/specifications/webrtc` and the later `vcr: "webrtc"`
  examples then issue no request at all. A cassette recorded in that state holds
  only what the cache was missing, and replays only in that one example order —
  which is how `spec/w3c/vcr_cassettes/webrtc.yml` came to lack the very request
  its own examples make. `spec/w3c/support/hal_cache.rb` clears the cache in a
  `before` hook so each example issues its own requests. **A cassette
  re-recorded without that reset is not reusable.**
- **`json` must stay below 3 (`relaton.gemspec`).** json 3.0.0 made the
  `JSON.parse` options keyword-only; Faraday's JSON response middleware — the
  one `Lutaml::Hal::Client#create_connection` installs — still passes them
  positionally, up to and including faraday 2.14.3. With json 3 every
  api.w3.org response raises `ArgumentError: wrong number of arguments (given 2,
  expected 1)`, surfaced as `Lutaml::Hal::ParsingError`, and this flavor fetches
  nothing. `spec/w3c/relaton/w3c/faraday_json_spec.rb` guards the constraint.
