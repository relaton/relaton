...
To update the index test fixture (used by tests), run `rake spec:update_index_3gpp`. This downloads the published index from the https://github.com/relaton/relaton-data-3gpp[relaton-data-3gpp] repository, keeps a curated subset of it, and writes `spec/3gpp/fixtures/index-v2.zip`. The full index has ~88,000 rows, so the fixture is deliberately a subset; `tasks/index_fixture_3gpp.rb` documents which document groups it keeps and why.
