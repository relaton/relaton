...
The data fetcher builds a pubid-backed `index-v2`, whose rows are `Pubid::Calconnect::Identifier` objects serialized as `_type: pubid:calconnect:standard`. The legacy `index-v1` is no longer produced here; https://github.com/relaton/relaton-data-calconnect[relaton-data-calconnect] derives it from the v2 rows for released relaton v2 clients. Until that repository publishes an `index-v2.zip`, lookups still read `index-v1`.

The test fixture is `spec/calconnect/fixtures/index-v1.zip`, a copy of the published index. There is no rake task to refresh it yet; one arrives with the consumer half.
