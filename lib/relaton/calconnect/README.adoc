...
Lookups read a pubid-backed `index-v2`, whose rows are `Pubid::Calconnect::Identifier` objects serialized as `_type: pubid:calconnect:standard`. A reference is parsed into an identifier and matched against the index structurally, so a number matches exactly rather than as a substring: `CC/DIR 1000` finds nothing, `CC/DIR 10005` finds the one document, and `CC/A 0001` is not `CC/A 1`. An undated reference reaches every year of a document and the most recent is returned first.

The legacy `index-v1` is no longer produced here; https://github.com/relaton/relaton-data-calconnect[relaton-data-calconnect] rebuilds it from each crawled document's own docidentifier for released relaton v2 clients.

To refresh the index test fixture, run `rake spec:update_index_calconnect`. This downloads the published `index-v2.zip` from the https://github.com/relaton/relaton-data-calconnect[relaton-data-calconnect] repository.
