...
The index test fixture (used by tests) is the published pubid-structured `spec/fixtures/index-v2.zip` (rows are `_type: pubid:itu:…` hashes) from the `v2` branch of the https://github.com/relaton/relaton-data-itu-r[relaton-data-itu-r] repository. Refresh it by re-downloading that `index-v2.zip` (it must deserialize under the pinned pubid).
