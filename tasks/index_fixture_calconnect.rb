...
# Builds `spec/calconnect/fixtures/index-v2.zip`, the offline index the
# CalConnect suite searches against.
#
# Like ECMA's and OGC's, and unlike 3GPP's, this fixture is the **whole**
# published index, not a curated subset: CalConnect publishes 191 rows, small
# enough that keeping all of them costs nothing and removes any question of
# which rows a spec depends on.
#
# Rows are copied **verbatim** from `relaton-data-calconnect`'s published
# `index-v2.zip`, so the stored shapes are exactly what
# `Relaton::Calconnect::DataFetcher` writes and what the runtime deserializes.
#
# Pure logic lives here so `spec/tasks/` can unit test it; the
# `rake spec:update_index_calconnect` task is a thin wrapper.
module IndexFixtureCalconnect
  SOURCE = "https://raw.githubusercontent.com/relaton/relaton-data-calconnect/v2/index-v2.zip"
  ...
  class << self
    ...
    # A v1 index keys rows on a plain String (`"CC/DIR 10005:2019"`); a v2 row
    # keys on a pubid hash carrying `_type`. Writing v1 rows under the v2 name
